...
def background_hsv_gradient(img, t, hue0=10, hue1=140):
    ...
    hue = (hue0 + (hue1 - hue0) * ys + 10*np.sin(t*1.2 + ys*2.0)).astype(np.float32)
...
def scene_lissajous(img, t):
    ...
    col = hsv_to_bgr(int(140 + 40*np.sin(t*2.5)), 230, 250)
...
def scene_rose_polar(img, t):
    ...
    col = hsv_to_bgr(int(20 + 35*np.sin(t*1.8)), 230, 255)
    cv2.polylines(img, [pts], False, col, 2, cv2.LINE_AA)
    # Círculos “beats”
    for i in range(8):
        r = int(22 + 15*np.sin(t*6.0 + i*0.8))
        cv2.circle(img, (int(W*0.12 + i*90), int(H*0.82)), max(1, r), (150,250,150), 2, cv2.LINE_AA)
...
def scene_spirograph(img, t):
    ...
    col = hsv_to_bgr(int(80 + 90*(0.5+0.5*np.sin(t*1.5))), 255, 230)
    cv2.polylines(img, [pts], False, col, 2, cv2.LINE_AA)
...
def scene_particles(img, t, rng):
    ...
    col = hsv_to_bgr(int(95 + 40*math.sin(t*2.0)), 210, int(210 + 40*v))
...
def scene_fire(img, t, state):
    ...
    # Mapeo a color con HSV (azul->celeste->blanco)
    h = (120 - 20*np.clip(heat, 0, 1)).astype(np.uint8)     # 120..100 (Azul/Cian)
